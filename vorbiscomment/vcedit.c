...
static int page_buffer_push(vcedit_buffer_chain *bufferlink, ogg_page *og) {
  ...
  tmp = realloc(buffer->data, buffer->data_len + og->header_len + og->body_len);
  if (tmp) {
    buffer->data = tmp;
    memcpy(buffer->data + buffer->data_len, og->header, og->header_len);
    buffer->data_len += og->header_len;
    memcpy(buffer->data + buffer->data_len, og->body, og->body_len);
    result = 1;
...
static int buffer_chain_newlink(vcedit_state *state) {
  ...
  if (result > 0) {
    ...
    state->lasterror = _("Couldn't get enough memory for input buffering.");
...
static int vcedit_add_serial(vcedit_state *state, long serial) {
  ...
  if (vcedit_contains_serial(state, serial)) {
    ...
    if (tmp) {
      ...
      *(state->serials.streams + state->serials.streams_len) = serial;
...
char *vcedit_error(vcedit_state *state) { return state->lasterror; }

vorbis_comment *vcedit_comments(vcedit_state *state) { return state->vc; }
...
int vcedit_open_callbacks(vcedit_state *state, void *in,
  ...
  if (!state->os) {
    state->lasterror =
        _("Ogg bitstream does not contain a supported data-type.");
    goto err;
  }
...
int vcedit_write(vcedit_state *state, void *out) {
  ...
  if (state->extrapage) {
    ...
    if (state->write(ogin.header, 1, ogin.header_len, out) !=
        (size_t)ogin.header_len) {
      goto cleanup;
    }
...
cleanup:
  ...
  if (!state->eosin) {
    state->lasterror = _("Error writing stream to output. "
